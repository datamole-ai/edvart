...
class GroupAnalysis(Section):
    ...
    def within_group_descriptive_stats(
        ...
        """
        GroupAnalysis.within_group_stats(
            df=df,
            groupby=groupby,
            column=column,
            stats=GroupAnalysis.default_group_descriptive_stats(),
    ...
    def within_group_quantile_stats(
        ...
        GroupAnalysis.within_group_stats(
            ...
            groupby=groupby,
            column=column,
            stats=GroupAnalysis.default_group_quantile_stats(),
            round_decimals=round_decimals,
        )
    ...
    def within_group_stats(
        ...
        """
        if stats is None:
            GroupAnalysis.within_group_descriptive_stats(df, groupby, column, round_decimals)
            GroupAnalysis.within_group_quantile_stats(df, groupby, column, round_decimals)
            return
    ...
    def group_analysis(
        ...
        if show_group_missing_values:
            ...
            GroupAnalysis.group_missing_values(df, groupby)
        ...
        for col in columns:
            ...
            if datatype == DataType.NUMERIC:
                if show_within_group_statistics:
                    GroupAnalysis.within_group_stats(df, groupby, col)
                if show_distribution_plots:
                    GroupAnalysis.overlaid_histograms(df, groupby, col)
            else:
                GroupAnalysis.group_barplot(df, groupby, col)
    ...
    def required_imports(self) -> List[str]:
        ...
        if self.verbosity == Verbosity.MEDIUM:
            return [
                total_dedent(
                    f"""
                        from edvart.report_sections.group_analysis import {ga}
                        group_missing_values = {ga}.group_missing_values
                        within_group_stats = {ga}.within_group_stats
                        within_group_descriptive_stats = {ga}.within_group_descriptive_stats
                        within_group_quantile_stats = {ga}.within_group_quantile_stats
                        group_barplot = {ga}.group_barplot
                        overlaid_histograms = {ga}.overlaid_histograms
                        default_group_quantile_stats = {ga}.default_group_quantile_stats
                        default_group_descriptive_stats = {ga}.default_group_descriptive_stats
                    """
                )
            ]
        # verbosity HIGH
    ...
    def _add_function_defs(self, cells: List[Dict[str, Any]]):
        ...
        code = (
            ...
            + "\n\n"
            + (get_code(GroupAnalysis.within_group_descriptive_stats).replace("GroupAnalysis.", ""))
            + "\n\n"
    ...
    def add_cells(self, cells: List[Dict[str, Any]], df: pd.DataFrame) -> None:
        ...
        if self.verbosity == Verbosity.LOW:
            if self.columns is None:
                ...
                code = f"group_analysis(df=df, groupby={self.groupby}, columns={self.columns})"
    ...
    def show(self, df: pd.DataFrame) -> None:
        ...
        if self.show_missing_vals:
            display(Markdown("## Missing values for each group"))
            GroupAnalysis.group_missing_values(df, self.groupby)
        ...
        for col in columns:
            ...
            if datatype == DataType.NUMERIC:
                if self.show_statistics:
                    GroupAnalysis.within_group_stats(df, self.groupby, col)
                if self.show_dist:
                    GroupAnalysis.overlaid_histograms(df, self.groupby, col)
            else:
                GroupAnalysis.group_barplot(df, self.groupby, col)
