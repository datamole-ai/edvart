...
class RollingStatistics(Section):
    ...
    def required_imports(self) -> List[str]:
        ...
        if self.verbosity <= 1:
            return [
                total_dedent(
                    """
                    from edvart.report_sections.timeseries_analysis import RollingStatistics
                    rolling_statistics = RollingStatistics.rolling_statistics
                    """
                )
            ]
        return [
    ...
    def show(self, df: pd.DataFrame) -> None:
        ...
        RollingStatistics.rolling_statistics(
            df=df, columns=self.columns, window_size=self.window_size
        )
