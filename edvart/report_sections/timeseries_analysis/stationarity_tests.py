...
class StationarityTests(Section):
    ...
    def stationarity_tests(
        ...
        stat_tests = StationarityTests.default_stationarity_tests()
        if not kpss_const:
    ...
    def required_imports(self) -> List[str]:
        ...
        if self.verbosity <= Verbosity.MEDIUM:
            return [
                total_dedent(
                    """
                    from edvart.report_sections.timeseries_analysis import StationarityTests
                    stationarity_tests = StationarityTests.stationarity_tests
                    """
                )
            ]
        return [
    ...
    def add_cells(self, cells: List[Dict[str, Any]]) -> None:
        ...
        if self.verbosity <= Verbosity.MEDIUM:
            ...
            code = (
                get_code(StationarityTests.default_stationarity_tests)
                + "\n\n"
                + get_code(StationarityTests.stationarity_tests).replace("StationarityTests.", "")
                + "\n\n"
                + default_call
            )
    ...
    def show(self, df: pd.DataFrame) -> None:
        ...
        StationarityTests.stationarity_tests(df=df, columns=self.columns)
