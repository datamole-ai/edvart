...
class FourierTransform(Section):
        ...
        if columns is None:
            ...
            for col in columns:
                if not is_numeric(df[col]):
                    raise ValueError(
                        f"Cannot perform Fourier transform for non-numeric column `{col}`"
                    )
    ...
    def required_imports(self) -> List[str]:
        ...
        if self.verbosity <= Verbosity.MEDIUM:
            return [
                total_dedent(
                    """
                    from edvart.report_sections.timeseries_analysis import FourierTransform
                    fourier_transform = FourierTransform.fourier_transform
                    """
                )
            ]
        return [
    ...
    def show(self, df: pd.DataFrame) -> None:
        ...
        FourierTransform.fourier_transform(
            df=df, sampling_rate=self.sampling_rate, columns=self.columns
        )
